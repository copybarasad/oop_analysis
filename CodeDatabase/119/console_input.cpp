...
GameCommand ConsoleInput::parseCommandString(const std::string& cmdStr) {
    ...
    else if (cmdStr == "START") { cmd.type = CommandType::Start; }
    else if (cmdStr == "QUIT") { cmd.type = CommandType::Quit; }
    else if (cmdStr == "1") { cmd.type = CommandType::UpgradeHp; }
    else if (cmdStr == "2") { cmd.type = CommandType::UpgradeHand; }
    else if (cmdStr == "3") { cmd.type = CommandType::UpgradeSpell; }
    else if (cmdStr == "back") { cmd.type = CommandType::BackMenu; }
    return cmd;
}
void ConsoleInput::loadDefaults() {
    ...
    keyMap["1"] = parseCommandString("1");
    keyMap["2"] = parseCommandString("2");
    keyMap["3"] = parseCommandString("3");
    keyMap["back"] = parseCommandString("back");
}

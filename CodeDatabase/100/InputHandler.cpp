...
char InputHandler::getInput() {
    ...
    while (true) {
        ...
        // Проверяем допустимые клавиши
        if (input == 'W' || input == 'A' || input == 'S' || input == 'D' || 
            input == 'Q' || input == 'C' || input == 'P' || input == 'K' || input == 'L') {
            std::cin.ignore(10000, '\n'); 
            return input;
        } else {
            std::cout << Colors::RED << "Invalid input! Use only: W, A, S, D, Q, X, C" << Colors::RESET << std::endl;
            std::cin.clear(); 
            std::cin.ignore(10000, '\n');   
        }
    }
...
bool InputHandler::showTargetsInRange(const Player& player, const GameState& state, int range) {
...
void InputHandler::spellProcessing(Player& player, const PlayerHand& hand, GameState& state) {
...
void InputHandler::processInput(Game& game) {
    ...
    Player& player = game.getGameState().getPlayer(); // или добавьте геттер для state в Game
    const PlayerHand& hand = game.getGameState().getHand();
    const GameField& field = game.getGameState().getField();
    ...
    switch (input) {
        ...
        case 'Q':
            ...
            break;
        case 'C': 
            spellProcessing(player, hand, state);
            break;
        case 'P':
            game.requestPause();
            break;

...
// читаем целое в диапазоне, с дефолтом
 int read_int(const std::string& prompt, int lo, int hi, int defVal) {
    for (;;) {
...
// читаем одну позицию (x y)
 Position read_pos_single(const std::string& prompt) {
    while (true) {
        std::cout << prompt << " (x y): ";
...
// читаем список позиций, с валидацией, без дублей
 std::vector<Position> read_positions_list(const std::string& title,
                                                 int w, int h)
{
    std::cout << title << "\n";
...
GameParams prompt_game_params() {
    ...
    // поле
    p.setWidth(read_int("Ширина поля", 10, 25, p.getWidth()));
    p.setHeight(read_int("Высота поля", 10, 25, p.getHeight()));
    ...
    // объекты
    p.spawners = read_positions_list("Координаты СПАВНЕРОВ", p.getWidth(), p.getHeight());
    p.walls    = read_positions_list("Координаты СТЕН (непроходимые клетки)", p.getWidth(), p.getHeight());
    p.slows    = read_positions_list("Координаты SLOW-клеток (пропуск следующего хода)", p.getWidth(), p.getHeight());

    return p;
}

 bool validate_no_conflicts(const GameParams& p) {
    auto in_bounds = [&](const Position& q){
        return (0 <= q.getX() && q.getX() < p.getWidth() && 0 <= q.getY() && q.getY() < p.getHeight());
    };
                             ...
                             std::vector<Spawner>& outSpawners)
{
    ...
    outField = Field{p.getWidth(), p.getHeight()};

...
void JsonManager::unmarshal(KeysConf& keys, const char* path) {
    try {
        nlohmann::json j = fileManager.read(path);
        std::unordered_set<sf::Keyboard::Scancode> used;
        int keysCount = 17;
        int keysInFile = j.size();
        // Если клавиш не столько же, сколько надо, точно что то не так
        if (keysCount != keysInFile) {
            throw KeyConfigException("Invalid number of key codes defined");
        }
        
        // Если столько сколько надо, но какие-то 2 раза, то какая то не найдется и будет ошибка
        keys.UpKey = j.at("UpKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.UpKey);
        keys.DownKey = j.at("DownKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.DownKey);
        keys.LeftKey = j.at("LeftKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.LeftKey);
        keys.RightKey = j.at("RightKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.RightKey);
        keys.ChangeKey = j.at("ChangeKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.ChangeKey);
        keys.OneKey = j.at("OneKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.OneKey);
        keys.TwoKey = j.at("TwoKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.TwoKey);
        keys.ThreeKey = j.at("ThreeKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.ThreeKey);
        keys.FourKey = j.at("FourKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.FourKey);
        keys.FiveKey = j.at("FiveKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.FiveKey);
        keys.CancelKey = j.at("CancelKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.CancelKey);
        keys.ConfirmKey = j.at("ConfirmKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.ConfirmKey);
        keys.LoadKey = j.at("LoadKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.LoadKey);
        keys.NewKey = j.at("NewKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.NewKey);
        keys.FinishKey = j.at("FinishKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.FinishKey);
        keys.SaveKey = j.at("SaveKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.SaveKey);
        keys.CloseKey = j.at("CloseKey").get<sf::Keyboard::Scancode>();
        used.insert(keys.CloseKey);

        if (used.size() != keysCount) {
            throw KeyConfigException("Some key codes have the same meaning");
        }
        for (auto& k : used) {
            if (k < 0 || k > 145) {
                throw KeyConfigException("Invalid key code value: " + std::to_string(k));
            }
        }
    }
    catch(const std::exception& e) {
        throw InvalidJson("unmarshal(KeyConfig)", e.what());
    }
}

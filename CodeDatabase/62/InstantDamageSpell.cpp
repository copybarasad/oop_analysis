...
bool InstantDamageSpell::use(GameField& gameField, int x, int y, int current_dist, int& player_boost_level_) {
    ...
    if(!enemies.size() && !towers.size()) return false;
...
bool InstantDamageSpell::use(GameField& gameField, Tower* tower) {
    std::pair<int, int> pos = gameField.getTowerCell(tower);
